...
class TestAccessControl(LiveServerTestCase):
    ...
    def test_authentication_page(self):
		page = self.page
        page.get(self.live_server_url + self.homepage)
		page.login()
	...
	def test_correct_admin_credentials(self):
		...
		# workaround page.get_homepage
		
		self.assertEqual(self.driver.current_url, self.live_server_url +
                self.homepage)

        with self.assertRaises(NoSuchElementException):
            page.get_incorrect_login_error()
    ...
    def test_incorrect_admin_credentials(self):
        ...
        self.assertNotEqual(page.get_incorrect_login_error(), None)
